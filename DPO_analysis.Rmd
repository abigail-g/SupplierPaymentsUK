...
# Apply the function to each row and duplicate records. Remove companies having na values
company_data <- company_data %>%
  rowwise() %>%
...
n_distinct(payment_times_1$Company)

na_rows <- payment_times[!complete.cases(payment_times),] %>% select(Company) %>% distinct(Company, .keep_all = TRUE)

payment_times_1 <- payment_times %>% filter(!Company %in% na_rows$Company)
payment_times <- na.omit(payment_times)

sum(is.na(payment_times))
sum(is.na(payment_times_1))

payment_times_1 <- payment_times_1 %>% filter(!grepl("2024",date))
...
payment_times %>%
  ggplot(aes(date, y=Average_time_to_pay)) + geom_point(alpha=0.5) + labs(x="Time / Month", y="time to pay")
...
n_distinct(master_data_distinct$Company)
n_distinct(payment_times$Company)
sum(is.na(payment_times))

```

#creating the master data set using payment_times_1

```{r}

master_data_1 <- payment_times_1 %>% 
  left_join( uk_bank_rate, by=c('date' = 'date')) %>%
  left_join( uk_BCI, by=c('date' = 'date')) %>%
  left_join( uk_gdp, by=c('date' = 'date')) %>%
  left_join( uk_inflation, by=c('date' = 'date')) %>%
  ...
  left_join(uk_interest_rate, by=c('date' = 'Date')) %>%
  mutate(date = my(date)) %>%
  filter(!Company %in% companies_with_na$company_name)


master_data_1 <- master_data_1 %>%
  left_join(company_data, by=c('Company' = 'company_name', 'date' = 'date')) %>%
  rename('interest_rate' = `interest rate`) %>%
  mutate('debt_to_equity' = liabilities/equity) %>%
  select(-liabilities, - equity)

na_rows <- master_data_1[!complete.cases(master_data_1),] %>% select(Company) %>% distinct(Company, .keep_all = TRUE)

sum(is.na(master_data_1))

#master_data_na_omit_1 <-na.omit(master_data_1)

master_data_na_omit_1 <- master_data_1 %>% filter(!Company %in% na_rows$Company)

master_data_na_omit_1$sector <- as.factor(master_data_na_omit_1$sector)
master_data_na_omit_1$Company <- as.factor(master_data_na_omit_1$Company)
str(master_data_na_omit_1)

master_data_distinct_1 <- master_data_na_omit_1 %>% distinct(Company, date, .keep_all = TRUE)
write.csv(master_data_distinct_1, file = "master_data_1.csv")

n_distinct(master_data_distinct_1$Company) #1332
n_distinct(payment_times_1$Company) #7860
...
```{r}
library(vtable)
st(master_data_distinct_1, vars= c("Average_time_to_pay", "bank_rate", "BCI", "GDP", "inflation_rate", "unemployment_rate", "interest_rate", "total_assets", "ROTA", "working_capital", "creditors_payment", "liquidity_ratio", "debt_to_equity"))

summary(master_data_distinct_1)

master_data_distinct <- master_data_distinct_1
...
master_data %>% group_by(Average_time_to_pay) %>% summarise(COUNT = n())
master_data_1 %>% group_by(Average_time_to_pay) %>% summarise(COUNT = n())
...
master_data_distinct_1 <- filter(master_data_distinct_1, master_data_distinct_1$Average_time_to_pay<2000)
...
lowerCor(master_data_distinct[, c("bank_rate", "BCI", "GDP", "inflation_rate", 
                                      "unemployment_rate", "interest_rate", 
                                      "total_assets", "ROTA", "debt_to_equity")])

lowerCor(master_data_distinct[, c("BCI", "GDP","inflation_rate",
                                      "unemployment_rate", "interest_rate", 
                                      "total_assets", "ROTA", "debt_to_equity")])
...
fixed_effects_model <- plm(Average_time_to_pay ~ BCI+GDP+unemployment_rate+inflation_rate+interest_rate+total_assets+ROTA+debt_to_equity+sector, 
                           data = master_data_distinct_1, 
                           index = c("Company","date"), 
                           model = "within")
