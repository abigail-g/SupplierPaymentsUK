...
master_data_new <- rename(master_data_new, 'interest_rate' = `interest rate`)

master_data <- master_data %>%
  mutate('debt_to_equity' = liabilities/equity) %>%
  select(-liabilities, - equity)

master_data_new <-na.omit(master_data)

master_data_new$sector <- as.factor(master_data_new$sector)
master_data_new$Company <- as.factor(master_data_new$Company)
str(master_data_new)
...
```{r}
master_data_new <- filter(master_data_new, master_data_new$Average_time_to_pay<2000)

ggplot(master_data_new, aes(x=interest_rate, y=bank_rate)) + geom_point() + labs(x="Credit Spread", y="Bank rate") + geom_vline(xintercept=2.5, lty=3)
...
```{r}
cor_matrix <- cor(master_data_new[, c("bank_rate", "BCI", "GDP", "inflation_rate", 
...
```{r}
compcount_by_sector <- master_data_new_1 %>%
  group_by(sector) %>%
...
master_data_agri <- master_data_new_1 %>%
...
master_data_construct <- master_data_new_1 %>%
...
```{r}
master_data_new_1 <- master_data_new %>% distinct(Company, date, .keep_all = TRUE)

payment_times_by_meco_fact <- lm(Average_time_to_pay~BCI+GDP+inflation_rate+unemployment_rate+interest_rate+total_assets+ROTA+debt_to_equity+sector, data=master_data_new_1)
